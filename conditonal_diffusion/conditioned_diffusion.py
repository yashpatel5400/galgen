    ...
    def __init__(self, path, transform=None):
        ...
        for filename in os.listdir(self.path):
            if filename.endswith(".png"):
                ...
                self.labels.append(float(filename.split("_")[3]))
...
class ContextUnet(nn.Module):
    def __init__(self, in_channels, n_feat=256, n_conditions=1):
        ...
        self.contextembed1 = EmbedFC(n_conditions, 4 * n_feat)
        self.contextembed2 = EmbedFC(n_conditions, 2 * n_feat)
        self.contextembed3 = EmbedFC(n_conditions, 1 * n_feat)
...
class DDPM(nn.Module):
    ...
    def sample(self, n_sample, size, device, guide_w=0.0):
        ...
        c_i = torch.rand(1,dtype=torch.float32).to(device)
        c_i += 10.
        c_i_store = c_i.detach().cpu().numpy()

        # double the batch
        c_i = c_i.repeat(2*n_sample)
        ...
        return x_i, x_i_store, c_i_store
...
def train_mnist():
    ...
    ddpm = DDPM(nn_model=ContextUnet(in_channels=3, n_feat=n_feat, n_conditions=n_conditions), betas=(1e-4, 0.02), n_T=n_T,
    ...
    dataset = MyDataset("./MW_dataset",transform=tf)
    ...
    for ep in range(n_epoch):
        ...
        if (ep+1) % 10 == 0:
            ddpm.eval()
            with torch.no_grad():
                n_sample = 4 * n_conditions
                for w_i, w in enumerate(ws_test):
                    x_gen, x_gen_store, c_store = ddpm.sample(n_sample, (3,128,128), device, guide_w=w)

                    # append some real images at bottom, order by class also
                    x_real = torch.Tensor(x_gen.shape).to(device)
                    for k in range(n_conditions):
                        for j in range(int(n_sample / n_conditions)):
                            try:
                                idx = torch.squeeze((c == k).nonzero())[j]
                            except:
                                idx = 0
                            x_real[k + (j * n_conditions)] = x[idx]

                    x_all = torch.cat([x_gen, x_real])
                    #grid = make_grid(x_all * -1 + 1, nrow=10)
                    grid = make_grid(x_all, nrow=10)
                    c_store = str(c_store[0].round(2))
                    save_image(grid, save_dir + f"image_ep{ep}_w{w}_{c_store}.png".format())
